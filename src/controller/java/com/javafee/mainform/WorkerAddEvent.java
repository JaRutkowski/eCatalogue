	...
	private void registerNow() {
		if (Validator.validateWorkerPesel(workerAddModFrame.getWorkerDataPanel().getTextFieldPeselNumber().getText())) {
			try {
				...
				for (UserData u : ud) {
					if (u.getLogin() != null && u.getLogin()
							.equals(workerAddModFrame.getWorkerDataPanel().getTextFieldLogin().getText()))
						result = false;
				}
				if (result) {
			...
			if (registrationEvent != null)
				Utils.displayOptionPane(
						SystemProperties.getInstance().getResourceBundle().getString("startForm.registrationSuccess3"),
						SystemProperties.getInstance().getResourceBundle()
